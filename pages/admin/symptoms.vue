...
<div class="container">
  <form action="/action_page.php">
    <div class="form-group">
      <label for="email">Symptom Title:</label>
    ...
    <div class="w-100 mt-4" v-if="symptoms.length > 0">
        <div class="w-100 p-2 bd-bottom d-flex justify-content-between align-items-center" v-for="(symptom, i) in symptoms" :key="i">
            <p>{{symptom.title}}</p>
            <div>
                <button @click="key = symptom.key, title = symptom.title, body = symptom.body, update = true" class="btn p-0 mdi mdi-pencil"></button>
...
export default {
    ...
    methods: {
        ...
        deleteSymptom(symptom) {
            this.firebase_remove_one('symptoms/'+symptom.key, (res) => {
                let notification = {
        ...
        upload() {
            if(this.title && this.body) {
                ...
                this.firebase_push_db({ ref: "symptoms", obj }, (res) => {
